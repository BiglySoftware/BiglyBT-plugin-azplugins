...
$Author: gouss $
$Id: PircBot.java,v 1.11 2006-04-13 20:59:33 gouss Exp $
...
public abstract class PircBot implements ReplyConstants {
     ...
     */
    public static final String VERSION = "1.4.5 (BiglyBT)";
    ...
    protected void handleLine(String line) {
        ...
        if (command.equals("PRIVMSG") && line.indexOf(":\u0001") > 0 && line.endsWith("\u0001")) {
            ...
            if (request.equals("VERSION")) {
            ...
            else if (request.toUpperCase().startsWith("FLASH")) {
                // FLASH request
            	String message = "";
            	try {
            		message = request.substring(6);
            	}
            	catch (Exception e) { }
                this.onFLASH(sourceNick, sourceLogin, sourceHostname, target, message);
            }
            else if (request.toUpperCase().startsWith("SET ")) {
                // SET request
            	String[] sSettings = request.substring(4).split(" ");
            	int[] settings = new int[sSettings.length];
            	for (int i=0;i<sSettings.length;i++)
            		settings[i] = Integer.parseInt(sSettings[i]);
                this.onSET(sourceNick, sourceLogin, sourceHostname, target, settings);
            }
            else if (request.toUpperCase().startsWith("STATIC ")) {
                // STATIC request
            	String connection = request.substring(8, request.indexOf("\"", 8));
            	String[] paras = request.substring(request.lastIndexOf("\"") + 2).split(" ");
                this.onSTATIC(sourceNick, sourceLogin, sourceHostname, target, connection, paras);
            }/*
            else if (request.toUpperCase().startsWith("RUN ")) {
                // RUN request
            	String runCommand = request.substring(4);
                this.onRUN(sourceNick, sourceLogin, sourceHostname, target, runCommand);
            }*/
            else if (request.toUpperCase().startsWith("INFO")) {
                // INFO request
            	int code;
            	try {
            		code = Integer.parseInt(request.substring(5));
            		if ((code > 255) || (code < 0))
            			code = 255;
            	}
            	catch (Exception ex)
                ...
                this.onINFO(sourceNick, sourceLogin, sourceHostname, target, code);
            }
    ...
    protected void onINFO(String sourceNick, String sourceLogin, String sourceHostname, String target , int code) {
    ...
    private long _messageDelay = 3000;
